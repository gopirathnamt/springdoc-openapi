...
public class SpringDocConfigPathsTest extends AbstractSpringDocTest {
	...
	void should_display_swaggerui_page() throws Exception {
		mockMvc.perform(get("/context-path/servlet-path/test/swagger.html").contextPath("/context-path")
		                                                                   .servletPath("/servlet-path"))
		       .andExpect(status().isFound());
		mockMvc.perform(get("/context-path/servlet-path/test/swagger-ui/index.html").contextPath("/context-path")
		                                                                            .servletPath("/servlet-path"))
		       .andExpect(status().isOk())
		       .andExpect(content().string(containsString("Swagger UI")));

		MvcResult mvcResult = mockMvc.perform(
				                             get("/context-path/servlet-path/test" + Constants.SWAGGER_INITIALIZER_URL).contextPath("/context-path")
				                                                                                                       .servletPath("/servlet-path"))
