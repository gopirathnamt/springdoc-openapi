...
public class SpringDocApp186Test extends AbstractCommonTest {
	...
	void testApp() throws Exception {
		webTestClient.get().uri(Constants.DEFAULT_API_DOCS_URL).exchange()
		             .expectStatus().isOk()
		             .expectBody().json(getContent("results/3.0.1/app186.json"), true);
	}

	@Test
	void testGroupActuatorAsCodeCheckBackwardsCompatibility() {
		webTestClient.get()
		             .uri(Constants.DEFAULT_API_DOCS_URL + "/group-actuator-as-code-check-backwards-compatibility")
		             .exchange()
		             .expectStatus().isOk()
		             .expectBody().json(getContent("results/3.0.1/app186.json"), true);
	}
	...
	@ComponentScan(basePackages = {"org.springdoc", "test.org.springdoc.api.v30.app186"})
