	...
	RouterFunction<?> routes(BookRepository br) {
		return
				RouterFunctions.nest(RequestPredicates.path("/greeter").and(RequestPredicates.path("/greeter2")),
				                     RouterFunctions.route(RequestPredicates.GET("/books").and(RequestPredicates.accept(
						                                           MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML)),
				                                           req -> ok().body(br.findAll()))
				                                    .and(route(RequestPredicates.GET("/books")
				                                                                .and(RequestPredicates.accept(
						                                                                MediaType.APPLICATION_XML,
						                                                                MediaType.TEXT_PLAIN)),
				                                               req -> ok().body(br.findAll())))
				                                    .andRoute(RequestPredicates.GET("/books/{author}"),
				                                              req -> ok().body(
						                                              br.findByAuthor(req.pathVariable("author")))));
	}
	...
	RouterFunction<?> routes1(BookRepository br) {
		return
				RouterFunctions.nest(RequestPredicates.accept(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML),
				                     RouterFunctions.route(RequestPredicates.GET("/books"),
				                                           req -> ServerResponse.ok().body(br.findAll()))
				                                    .andRoute(RequestPredicates.GET("/books/{author}"),
				                                              req -> ServerResponse.ok().body(br.findByAuthor(
	...
	RouterFunction<?> routes3(BookRepository br) {
		return
				nest(RequestPredicates.path("/greeter").or(RequestPredicates.path("/greeter2")),
				     route(RequestPredicates.GET("/books").and(RequestPredicates.accept(MediaType.APPLICATION_JSON,
				                                                                        MediaType.APPLICATION_XML)),
				           req -> ok().body(br.findAll()))
						     .and(route(RequestPredicates.GET("/books")
