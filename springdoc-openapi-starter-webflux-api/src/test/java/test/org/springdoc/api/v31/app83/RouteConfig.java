	...
	RouterFunction<ServerResponse> routerFunction() {
		return route(GET("/coffees"), this::all)
				.withAttribute(OPERATION_ATTRIBUTE,
				               operationBuilder().beanClass(CoffeeService.class).beanMethod("getAllCoffees"))

				.and(route(GET("/coffees/{id}"), this::byId)
						     .withAttribute(OPERATION_ATTRIBUTE, operationBuilder().beanClass(CoffeeService.class)
						                                                           .beanMethod("getCoffeeById")))

				.and(route(GET("/coffees/{id}/orders"), this::orders)
						     .withAttribute(OPERATION_ATTRIBUTE, operationBuilder().beanClass(CoffeeService.class)
						                                                           .beanMethod(
								                                                           "getOrdersForCoffeeById")));
	}
